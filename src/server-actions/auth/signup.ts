...
export async function signup(prevState: SignupFormState, form: FormData) {
  ...
  const errors: Record<keyof (typeof prevState)["errors"], string> =
  ...
  const account = form.get("account")?.toString() as
    | AccountValueTypes
    | undefined;
  ...
  const acceptSubscription =
    form.get("accept_subscription")?.toString() === "on";
  ...
  if (!userService.validateUsername(username)) {
    errors.username =
      'Ник должен быть не меньше трех букв длиной, содержать только латинские буквы или цифры в нижнем регистре, начинаться и заканчиваться буквой, может содержаьь "-" или "_" в середине, но не два раза подряд';
  ...
  switch (account) {
    ...
    case "no_account":
      ...
      break;
    case "set_name":
      accountId = await accountService.createAccountWithNameForUser(
        userId,
        accountName!,
      );
      break;
  ...
  {
    const { accountIds } = await inviteService.maybeJoinPendingInvites(
      userId,
      email,
      phone,
      username,
    );
    const accounts = await mapFilterAsync(
      accountIds,
      accountService.getAccount.bind(accountService),
    );
