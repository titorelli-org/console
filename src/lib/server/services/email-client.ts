...
export class EmailClient {
  ...
  constructor() {
    this.transports = {
      ...
      "restore-password@titorelli.ru": createTransport({
        ...
        secure: false,
        auth: {
          user: "restore-password@titorelli.ru",
          pass: env.SMTP_PASS_RESTORE_PASSWORD,
        },
      }),
    } satisfies { [K in FromEmails]: Transporter };
  }
  ...
  public async sendHTML(
    ...
    if (!transport) throw new Error(`Can't send email from address "${from}"`);
