...
export class UserService {
        ...
        data: [
          {
            ...
            emailCorporate:
              emailCorporate === "unknown" ? null : emailCorporate,
            emailDisposable:
              emailDisposable === "unknown" ? null : emailDisposable,
          },
          {
            type: "phone",
            primary: true,
  ...
  async tryLogin(
    ...
    if (user) {
      ...
      return [rawPasswordHash === user.passwordHash, user.id];
    }
  ...
  getIdentityType(identity: string): IdentityTypes | null {
    if (this.emailValidationService.isEmail(identity)) return "email";

    if (this.formatPhoneNumber(identity)) {
      return "phone";
    }

    if (this.validateUsername(identity)) return "username";
  ...
  async getUserByIdentnty(identity: string) {
    ...
    switch (identityType) {
      case "email":
        return this.getUserByEmail(identity);
      case "phone":
        return this.getUserByPhone(identity);
      case "username":
        return this.getUserByUsername(identity);
      default:
        return null;
    }
  }
  ...
  async getUserByUsername(username: string) {
    if (!this.validateUsername(username)) return null;
  ...
  async getUserByEmail(email: string) {
    if (!this.emailValidationService.isEmail(email)) return null;
