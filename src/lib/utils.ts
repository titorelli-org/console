...
export function mapAsyncTry<T, U, E extends Error = Error>(
  ...
  return Promise.all(
    array.map(async (element, index, array) => {
      try {
...
export function mapFilter<T, U>(
  ...
  return array
    .map(callback)
    .filter((result): result is NonNullable<U> => result != null);
}
...
export async function mapFilterAsync<T, U>(
  ...
  return results.filter(
    (result): result is NonNullable<Awaited<U>> => result != null,
  );
}
