...
import {
  mapAccessTokenDtoToVm,
  type AccessTokenCreatedResultVm,
  type AccessTokenVm,
} from "@/types/access-tokens";
...
export const GET = createZodRoute()
  ...
  .handler(async (_req, { params }) => {
    ...
    if (
      !(await securityService.userHasAccessToAccountTokens(user.id, accountId))
    )
      unauthorized();
...
export const POST = createZodRoute()
  ...
  .handler(async (_req, { params, body: { name, description } }) => {
    ...
    const { token, id } = await accessTokensService.create(
      accountId,
      name,
      description,
    );
